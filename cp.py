...
class ConstraintProgramming():
    ...
    def get_cp_rhythm_idx(self, rhythm_tokens, rhythm_output, epoch, i, device, sample_key):
        return self._cpbp_java(rhythm_tokens, rhythm_output, epoch, i, device, sample_key, True)
    
    def get_cp_pitch_probs(self, pitch_tokens, pitch_output, epoch, i, device, sample_key):
        return self._cpbp_java(pitch_tokens, pitch_output, epoch, i, device, sample_key, False)
    
    def _cpbp_java(self, tokens, output, epoch, i, device, sample_key, cp_on_rhythm):
        ...
        with open(os.path.join(self.minicpbp_music_path, filename), 'w') as f, open('ML.txt', 'a') as f2, open('Token.txt', 'a') as f3:
            ...
            f3.write(i_title + '\n')
            for j in range(num_sample):
                # print(map(str, tokens[j].tolist()[:i]))
        ...
        # print(key, filename, num_sample, i, ml_weight)
        
        cmd = self._get_java_command(key, filename, num_sample, i, ml_weight, sample_key)
        print(cmd)
    ...
    def _get_java_command(self, key, filename, num_sample, i, ml_weight, sample_key):
        return self._get_java_command_rhythm(filename, num_sample, i, ml_weight, sample_key) if key is self.RHYTHM else self._get_java_command_pitch(filename, num_sample, i, ml_weight, sample_key)
    
    def _get_java_command_rhythm(self, filename, num_sample, i, ml_weight, sample_key):
        ...
        if cp_model_name == 'rhythmIncreasingReset':
            ...
            nb_bars_group = self.config[self.RHYTHM]['weight_variation']['nb_bars_group']
            return f'{self.BASIC_JAVA_CMD}{cp_model_name} {filename} {num_sample} {i} {ml_weight} {nb_bars_group}'.split()
        elif cp_model_name == 'rhythmPatternOccurrences':
            pattern_length = 0.125
            pattern_positions = [0, 0.25, 0.5, 0.75]
            pattern_positions = ','.join(str(element) for element in pattern_positions)
            nb_bars_group = self.config[self.RHYTHM]['weight_variation']['nb_bars_group']
            return f'{self.BASIC_JAVA_CMD}{cp_model_name} {filename} {num_sample} {i} {ml_weight} {nb_bars_group} {pattern_length} {pattern_positions}'.split()
        elif cp_model_name == 'rhythmPatternCorrelation':
            # pattern_length = 0.125
            # pattern_positions = [0, 0.25, 0.5, 0.75]
            # pattern_positions = ','.join(str(element) for element in pattern_positions)
            # nb_bars_group = self.config[self.RHYTHM]['weight_variation']['nb_bars_group']
            return f'{self.BASIC_JAVA_CMD}{cp_model_name} {filename} {num_sample} {i} {ml_weight}'.split()
        else:
            raise Exception(f'Not a valid cp model on {self.RHYTHM}')

    def _get_java_command_pitch(self, filename, num_sample, i, ml_weight, sample_key):
        ...
        elif cp_model_name == 'pitchPatternCorrelation':
            min_weight = self.config[self.PITCH]['weight_variation']['weight_min']
            # pattern_positions = [0, 0.25, 0.5, 0.75]
            return f'{self.BASIC_JAVA_CMD}{cp_model_name} {filename} {num_sample} {i} {self.FILENAME_TOKEN_RHYTHM} {min_weight} {str(sample_key)}'.split()
        else:
