...
class CMTtrainer(BaseTrainer):
    ...
    def train(self, **kwargs):
        ...
        logger.info(str(kwargs["restore_epoch"]))
        logger.info(str(kwargs["load_rhythm"]))
    ...
    def _epoch(self, epoch, mode, rhythm_only=False, **kwargs):
        ...
        for i, data in enumerate(loader):
            ...
            rhythm_loss = rhythm_criterion(rhythm_out, (data['rhythm'][:, 1:].contiguous().view(-1)).long())
...
def differing_locations(array1, array2):
    ...
    for i in range(len(array1)):
        if len(array1[i]) != len(array2[i]):
            print("The arrays have different number of columns at row " + str(i))
            return
